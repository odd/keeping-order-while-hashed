...
import reftree.core.RefTree.Null
...
object LinkedHashMap extends MutableMapFactory[LinkedHashMap] {
  ...
  val Empty = new LinkedHashMap[Nothing, Nothing]
  def empty[K, V] = Empty.asInstanceOf[LinkedHashMap[K, V]]

  import reftree.core._
  val z = RefTree.Ref(Empty, Seq.empty).rename("∅")
  implicit def `LinkedEntry RefTree`[K : ToRefTree, V : ToRefTree]: ToRefTree[LinkedEntry[K, V]] = { entry: LinkedEntry[K, V] =>
    entry match {
      case null =>
        new Null
      case e: LinkedEntry[K, V] if e.earlier == null =>
        RefTree.Ref(e, Seq(
          z.toField,
          e.key.refTree.toField,
          e.value.refTree.toField,
          e.later.refTree.toField)
        ).rename("Entry")
      case e: LinkedEntry[K, V] if e.later == null =>
        RefTree.Ref(e, Seq(
          e.earlier.key.refTree.toField,
          e.key.refTree.toField,
          e.value.refTree.toField,
          z.toField)
        ).rename("Entry")
      case e: LinkedEntry[K, V] =>
        RefTree.Ref(e, Seq(
          e.earlier.key.refTree.toField,
          e.key.refTree.toField,
          e.value.refTree.toField,
  ...
  implicit def `LinkedHashMap RefTree`[K : ToRefTree, V : ToRefTree]: ToRefTree[LinkedHashMap[K, V]] = { map: LinkedHashMap[K, V] =>
    if (map.firstEntry == null) z.rename("Ø")
    else {
      RefTree.Ref(map, Seq(
        map.firstEntry.refTree.toField.withName("firstEntry"),
        map.lastEntry.refTree.toField.withName("lastEntry"))
