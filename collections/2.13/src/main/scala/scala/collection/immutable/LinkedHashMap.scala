...
/** This class implements immutable maps using a Compressed Hash-Array Mapped Prefix-tree.
  ...
  *
  *  @since   2.13
  *  @define Coll `immutable.LinkedHashMap`
  *  @define coll immutable champ hash map
  */

final class LinkedHashMap[K, +V] private[immutable] (private[immutable] val rootNode: BitmapIndexedMapNode[K, V])
  ...
  private final class KeySet extends ImmutableKeySet {
    ...
    private[this] def newKeySetOrThis(newLinkedHashMap: LinkedHashMap[K, _]): Set[K] =
      if (newLinkedHashMap eq LinkedHashMap.this) this else newLinkedHashMap.keySet
    private[this] def newKeySetOrThis(newRootNode: BitmapIndexedMapNode[K, _]): Set[K] =
      if (newRootNode eq rootNode) this else new LinkedHashMap(newRootNode).keySet
    ...
    override def excl(elem: K): Set[K] = newKeySetOrThis(LinkedHashMap.this - elem)
    override def filter(pred: K => Boolean): Set[K] = newKeySetOrThis(LinkedHashMap.this.filter(kv => pred(kv._1)))
    override def filterNot(pred: K => Boolean): Set[K] = newKeySetOrThis(LinkedHashMap.this.filterNot(kv => pred(kv._1)))
  }
  ...
  @`inline` private[this] def newLinkedHashMapOrThis[V1 >: V](newRootNode: BitmapIndexedMapNode[K, V1]): LinkedHashMap[K, V1] =
    if (newRootNode eq rootNode) this else new LinkedHashMap(newRootNode)

  def updated[V1 >: V](key: K, value: V1): LinkedHashMap[K, V1] = {
    val keyUnimprovedHash = key.##
    newLinkedHashMapOrThis(rootNode.updated(key, value, keyUnimprovedHash, improve(keyUnimprovedHash), 0, replaceValue = true))
  }

  // preemptively overridden in anticipation of performance optimizations
  override def updatedWith[V1 >: V](key: K)(remappingFunction: Option[V] => Option[V1]): LinkedHashMap[K, V1] =
    super.updatedWith[V1](key)(remappingFunction)

  def removed(key: K): LinkedHashMap[K, V] = {
    val keyUnimprovedHash = key.##
    newLinkedHashMapOrThis(rootNode.removed(key, keyUnimprovedHash, improve(keyUnimprovedHash), 0))
  }

  override def concat[V1 >: V](that: scala.IterableOnce[(K, V1)]): LinkedHashMap[K, V1] = that match {
    case hm: LinkedHashMap[K, V1] => newLinkedHashMapOrThis(rootNode.concat(hm.rootNode, 0))
    case hm: collection.mutable.LinkedHashMap[K, V] =>
  ...
  override def tail: LinkedHashMap[K, V] = this - head._1

  override def init: LinkedHashMap[K, V] = this - last._1
  ...
  override def equals(that: Any): Boolean =
    that match {
      case map: LinkedHashMap[K, V] => (this eq map) || (this.rootNode == map.rootNode)
  ...
  override protected[this] def className = "LinkedHashMap"

  def merged[V1 >: V](that: LinkedHashMap[K, V1])(mergef: ((K, V), (K, V1)) => (K, V1)): LinkedHashMap[K, V1] =
    if (mergef == null) {
      that ++ this
    } else {
      if (isEmpty) that
        ...
        if (that.rootNode.containsKey(k, originalHash, improved, 0)) {
          ...
          new LinkedHashMap(that.rootNode.updated(mergedK, mergedV, mergedOriginalHash, mergedImprovedHash, 0, replaceValue = true))
        } else {
          new LinkedHashMap(that.rootNode.updated(k, v, originalHash, improved, 0, replaceValue = true))
        }
      } else if (that.size == 0) {
        ...
        if (rootNode.containsKey(k, thatOriginalHash, thatImproved, 0)) {
          ...
          val mergedOriginalHash = mergedK.##
          val mergedImprovedHash = improve(mergedOriginalHash)
          new LinkedHashMap(rootNode.updated(mergedK, mergedV, mergedOriginalHash, mergedImprovedHash, 0, replaceValue = true))
        } else {
          new LinkedHashMap(rootNode.updated(k, v, thatOriginalHash, thatImproved, 0, replaceValue = true))
        }
      } else {
        val builder = new LinkedHashMapBuilder[K, V1]
        rootNode.mergeInto(that.rootNode, builder, 0)(mergef)
        builder.result()
      }
    }

  override def transform[W](f: (K, V) => W): LinkedHashMap[K, W] =
    newLinkedHashMapOrThis(rootNode.transform(f)).asInstanceOf[LinkedHashMap[K, W]]

  override protected[collection] def filterImpl(pred: ((K, V)) => Boolean, isFlipped: Boolean): LinkedHashMap[K, V] = {
    val newRootNode = rootNode.filterImpl(pred, isFlipped)
    if (newRootNode eq rootNode) this
    else if (newRootNode.size == 0) LinkedHashMap.empty
    else new LinkedHashMap(newRootNode)
  }

  override def removedAll(keys: IterableOnce[K]): LinkedHashMap[K, V] = {
    if (isEmpty) {
      this
    } else {
      keys match {
  ...
  override def partition(p: ((K, V)) => Boolean): (LinkedHashMap[K, V], LinkedHashMap[K, V]) = {
  ...
  override def take(n: Int): LinkedHashMap[K, V] = {
    // This method has been preemptively overridden in order to ensure that an optimizing implementation may be included
    // in a minor release without breaking binary compatibility.
    //
  ...
  override def takeRight(n: Int): LinkedHashMap[K, V] = {
    // This method has been preemptively overridden in order to ensure that an optimizing implementation may be included
    // in a minor release without breaking binary compatibility.
    //
  ...
  override def takeWhile(p: ((K, V)) => Boolean): LinkedHashMap[K, V] = {
    // This method has been preemptively overridden in order to ensure that an optimizing implementation may be included
    // in a minor release without breaking binary compatibility.
    //
  ...
  override def dropWhile(p: ((K, V)) => Boolean): LinkedHashMap[K, V] = {
    // This method has been preemptively overridden in order to ensure that an optimizing implementation may be included
    // in a minor release without breaking binary compatibility.
    //
  ...
  override def dropRight(n: Int): LinkedHashMap[K, V] = {
    // This method has been preemptively overridden in order to ensure that an optimizing implementation may be included
    // in a minor release without breaking binary compatibility.
    //
  ...
  override def drop(n: Int): LinkedHashMap[K, V] = {
    // This method has been preemptively overridden in order to ensure that an optimizing implementation may be included
    // in a minor release without breaking binary compatibility.
    //
  ...
  override def span(p: ((K, V)) => Boolean): (LinkedHashMap[K, V], LinkedHashMap[K, V]) = {
    // This method has been preemptively overridden in order to ensure that an optimizing implementation may be included
    // in a minor release without breaking binary compatibility.
    //
...
private[immutable] sealed abstract class MapNode[K, +V] extends Node[MapNode[K, V @uV]] {
  ...
  def isFirst(index: Int): Boolean

  def getPreviousKey(index: Int): K

  def isLast(index: Int): Boolean

  def getNextKey(index: Int): K
  ...
  /** Merges this node with that node, adding each resulting tuple to `builder`
    *
    * `this` should be a node from `left` LinkedHashMap in `left.merged(right)(mergef)`
    *
    * @param that node from the "right" LinkedHashMap. Must also be at the same "path" or "position" within the right tree,
    *             as `this` is, within the left tree
    */
  def mergeInto[V1 >: V](that: MapNode[K, V1], builder: LinkedHashMapBuilder[K, V1], shift: Int)(mergef: ((K, V), (K, V1)) => (K, V1)): Unit
  ...
  def buildTo[V1 >: V](builder: LinkedHashMapBuilder[K, V1]): Unit
}
...
private final class BitmapIndexedMapNode[K, +V](
  ...
  */
  ...
  override def buildTo[V1 >: V](builder: LinkedHashMapBuilder[K, V1]): Unit = {
    var i = 0
    val iN = payloadArity
    val jN = nodeArity
    while (i < iN) {
  ...
  override def mergeInto[V1 >: V](that: MapNode[K, V1], builder: LinkedHashMapBuilder[K, V1], shift: Int)(mergef: ((K, V), (K, V1)) => (K, V1)): Unit = that match {
...
/** Used in LinkedHashMap[K, V]#removeAll(HashSet[K]) */
...
object LinkedHashMap extends MapFactory[LinkedHashMap] {
  ...
  @transient
  private final val EmptyMap = new LinkedHashMap(MapNode.empty)

  def empty[K, V]: LinkedHashMap[K, V] =
    EmptyMap.asInstanceOf[LinkedHashMap[K, V]]

  def from[K, V](source: collection.IterableOnce[(K, V)]): LinkedHashMap[K, V] =
    source match {
      case hs: LinkedHashMap[K, V] => hs
  ...
  /** Create a new Builder which can be reused after calling `result()` without an
    ...
    */
  def newBuilder[K, V]: ReusableBuilder[(K, V), LinkedHashMap[K, V]] = new LinkedHashMapBuilder[K, V]
}


/** A Builder for a LinkedHashMap.
  * $multipleResults
  */
private[immutable] final class LinkedHashMapBuilder[K, V] extends ReusableBuilder[(K, V), LinkedHashMap[K, V]] {
  import MapNode._
  import Node._
  ...
  /** The last given out LinkedHashMap as a return value of `result()`, if any, otherwise null.
    * Indicates that on next add, the elements should be copied to an identical structure, before continuing
    * mutations. */
  private var aliased: LinkedHashMap[K, V] = _

  private def isAliased: Boolean = aliased != null

  /** The root node of the partially build LinkedHashMap */
  ...
  override def result(): LinkedHashMap[K, V] =
    if (rootNode.size == 0) {
      LinkedHashMap.empty
    } else if (aliased != null) {
      aliased
    } else {
      aliased = new LinkedHashMap(rootNode)
      releaseFence()
      aliased
    }
